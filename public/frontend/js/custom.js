(function ($) {
    ...
    // Rooms Package Slider
    var swiper = new Swiper(".rooms-trip-slider", {
        slidesPerView: 1,
        speed: 1500,
        spaceBetween: 24,
        autoplay: {
    ...
    var swiper = new Swiper(".traditional-houses-slider", {
        ...
        pagination: {
            el: ".traditional-houses-pagination",
            clickable: true,
        },
        breakpoints: {
            280: { slidesPerView: 1 },
    ...
    // OTP Verification - Resend Code with Countdown
    const resendBtn = document.getElementById('resendBtn');
    const resendText = document.getElementById('resendText');
    const countdownEl = document.getElementById('countdown');
    const resendMessage = document.getElementById('resendMessage');
    const otpEmailInput = document.querySelector('input[name="email"]');

    if (resendBtn && resendText && countdownEl && resendMessage && otpEmailInput) {
        const email = otpEmailInput.value;
        const STORAGE_KEY = 'otp_resend_expiry_' + email;
        let countdown = 60;
        ...
        function startCountdown(initialSeconds = 60) {
            ...
            resendText.innerHTML = 'Resend Code (<span id="countdown">' + countdown + '</span>s)';
            ...
            countdownInterval = setInterval(function () {
                ...
                // Update localStorage
                localStorage.setItem(STORAGE_KEY, Date.now() + (countdown * 1000));
        ...
        resendBtn.addEventListener('click', function () {
            ...
            // Get the resend route URL from data attribute or construct it
            const resendUrl = resendBtn.getAttribute('data-resend-url');

            if (!resendUrl) {
                console.error('Resend URL not found');
                resendMessage.innerHTML = '<div class="alert alert-danger alert-sm py-2">' +
                    '<i class="bi bi-exclamation-circle me-1"></i>Configuration error. Please contact support.</div>';
                resendBtn.disabled = false;
                resendText.textContent = 'Resend Code';
                return;
            }
            ...
            fetch(resendUrl, {
                ...
                .then(result => {
                    if (result.ok && result.data && result.data.success) {
                        resendMessage.innerHTML =
                            ...
                            '<i class="bi bi-check-circle me-1"></i>' + result.data.message + '</div>';
                        startCountdown(60); // Restart countdown with 60 seconds
                    } else {
                        const errorMessage = result.data && result.data.message ? result.data.message : 'Failed to resend OTP';
                ...
                .catch(error => {
                    ...
                    resendMessage.innerHTML = '<div class="alert alert-danger alert-sm py-2">' +
                        '<i class="bi bi-exclamation-circle me-1"></i>Failed to resend OTP. Please try again.</div>';
                    resendBtn.disabled = false;
                    resendText.textContent = 'Resend Code';
                });
        });
    ...
    // Booking Reschedule DateRangePicker Initialization (Inline Form)
    function initRescheduleDatePicker() {
        const $inputs = $('input[id^="rescheduleDate-"]');

        if (!$inputs.length) {
            return;
        }

        console.log('Initializing reschedule daterangepicker for ' + $inputs.length + ' booking(s)');

        $inputs.each(function () {
            const $input = $(this);
            const inputId = $input.attr('id');
            const bookingId = inputId.replace('rescheduleDate-', '');
            const $btn = $('#calendarBtn-' + bookingId);
            ...
            if ($input.data('daterangepicker')) {
                try {
                    ...
                    console.log('No existing daterangepicker to remove');
                }
            }
