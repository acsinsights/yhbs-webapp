<?php

namespace App\Notifications;

use Illuminate\Bus\Queueable;
use Illuminate\Notifications\Notification;
use App\Models\Booking;

class NewBookingNotification extends Notification
{
    use Queueable;

    public $booking;

    /**
     * Create a new notification instance.
     */
    public function __construct(Booking $booking)
    {
        $this->booking = $booking;
    }

    /**
     * Get the notification's delivery channels.
     *
     * @return array<int, string>
     */
    public function via(object $notifiable): array
    {
        return ['database'];
    }

    /**
     * Get the array representation of the notification.
     *
     * @return array<string, mixed>
     */
    public function toArray(object $notifiable): array
    {
        $bookingType = match ($this->booking->bookingable_type) {
            'App\Models\Room' => 'Room',
            'App\Models\House' => 'House',
            'App\Models\Boat' => 'Boat',
            default => 'Property',
        };

        $propertyName = $this->booking->bookingable?->name ?? 'Property';

        return [
            'booking_id' => $this->booking->id,
            'booking_number' => $this->booking->booking_id,
            'booking_type' => $bookingType,
            'customer_name' => $this->booking->user->name ?? 'Guest',
            'total_amount' => $this->booking->total_amount,
            'check_in' => $this->booking->check_in?->format('M d, Y'),
            'message' => "New {$bookingType} booking received for {$propertyName}",
            'icon' => 'o-calendar-days',
            'url' => $this->getBookingUrl(),
        ];
    }

    /**
     * Get the booking detail URL based on type
     */
    private function getBookingUrl(): string
    {
        $routePrefix = match ($this->booking->bookingable_type) {
            'App\Models\Room' => 'admin.bookings.room',
            'App\Models\House' => 'admin.bookings.house',
            'App\Models\Boat' => 'admin.bookings.boat',
            default => 'admin.bookings.room',
        };

        return route($routePrefix . '.show', $this->booking->id);
    }
}
